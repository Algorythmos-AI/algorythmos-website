...
// Expected values
const EXPECTED_DOMAIN = 'https://www.algorythmos.fr';
const EXPECTED_TITLE = 'Algorythmos™ — AI Consultancy for SMEs | AI That Deliver';
const EXPECTED_OG_IMAGE = 'https://www.algorythmos.fr/Algorythmos.png';
const BRAND_KEYWORDS = ['algorythmos', 'algorythmos ai', 'algorythmos france', 'algorythmos consulting'];
...
function validateStructuredData() {
  ...
  const orgSchema = jsonLdData.find(d => d['@type'] === 'Organization');
  if (orgSchema) {
    ...
    // Check URL domain
    if (orgSchema.url) {
      validateDomain(orgSchema.url, 'Organization schema URL');
    }
...
function validateSitemap() {
  ...
  if (urlMatches) {
    log.success(`Found ${urlMatches.length} URLs in sitemap`);
    
    let invalidUrls = 0;
    for (const match of urlMatches) {
      const url = match.replace(/<\/?loc>/g, '');
      if (!url.startsWith(EXPECTED_DOMAIN)) {
        log.error(`Invalid URL in sitemap: ${url}`);
        invalidUrls++;
        errors++;
      }
    }
    ...
    if (invalidUrls === 0) {
      log.success('All URLs use correct domain');
    }
  } else {
