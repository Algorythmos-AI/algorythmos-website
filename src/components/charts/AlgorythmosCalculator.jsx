...
const Label = ({ children, hint }) => (
  <div className="mb-1 flex items-center gap-2 text-sm font-medium text-slate-200 leading-tight">
    <span className="break-words hyphens-auto">{children}</span>
    {hint ? <span className="text-xs text-slate-400 break-words">{hint}</span> : null}
  </div>
);
  ...
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h3 className="text-base font-semibold">{t("ui.calculator.kpi.roi")}</h3>
        <div
          className="relative -mx-2 px-2 overflow-x-auto no-scrollbar snap-x snap-mandatory"
          role="tablist"
          aria-label={t("ui.aria.calculatorModes")}
        >
          <div className="flex items-center gap-2 min-w-full whitespace-nowrap">
            <SegBtn active={s.mode === "simple"} onClick={() => setS((x) => ({ ...x, mode: "simple" }))}>{t("ui.calculator.mode.simple")}</SegBtn>
            <SegBtn active={s.mode === "advanced"} onClick={() => setS((x) => ({ ...x, mode: "advanced" }))}>{t("ui.calculator.mode.advanced")}</SegBtn>
            <SegBtn active onClick={() => applyPreset("Starter")}>{t("ui.calculator.preset.small")}</SegBtn>
            <SegBtn active onClick={() => applyPreset("Growth")}>{t("ui.calculator.preset.medium")}</SegBtn>
            <SegBtn active onClick={() => applyPreset("Scale")}>{t("ui.calculator.preset.large")}</SegBtn>
            <SegBtn onClick={() => setS({
              ...s,
              mode: "advanced",
              baselineMode: "manual",
            ...
            })}>{t("ui.calculator.actions.reset")}</SegBtn>
            <SegBtn onClick={share}>{t("ui.calculator.actions.share")}</SegBtn>
            <SegBtn onClick={exportCSV}>{t("ui.calculator.actions.exportCsv")}</SegBtn>
            <Select
              value={s.currency}
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <div>
          <Label hint={t("ui.calculator.labels.monthlyVolumeHint")}>{t("ui.calculator.labels.monthlyVolume")}</Label>
          <NumberBox value={s.volume} onChange={(v) => setS((x) => ({ ...x, volume: Math.max(0, Math.round(v)) }))} allowFloat={false} />
        </div>

        <div>
          <div className="mb-1 flex items-center justify-between">
            <Label hint={t("ui.calculator.labels.currentCostHint")}>{t("ui.calculator.labels.currentCost")}</Label>
            <div className="flex gap-2">
              <SegBtn active={s.baselineMode === "manual"} onClick={() => { setS((x) => ({ ...x, baselineMode: "manual" })); track("baseline_switched", { to: "manual" }); }}>{t("ui.calculator.labels.manual")}</SegBtn>
              <SegBtn active={s.baselineMode === "computed"} onClick={() => { setS((x) => ({ ...x, baselineMode: "computed" })); track("baseline_switched", { to: "computed" }); }}>{t("ui.calculator.labels.computed")}</SegBtn>
            </div>
          </div>
          ...
          {s.baselineMode === "computed" && (
            <p className="mt-1 text-xs text-slate-400 break-words hyphens-auto">
              {t("ui.calculator.labels.computedExplanation")} = {fmtC(computeBaselineM({ V: s.volume, h: s.handleMin, w: s.hourly, overheadPct: s.overheadPct }))}
            </p>
          )}
        </div>
        ...
        {s.mode === "advanced" && (
          <>
            <div className="md:col-span-2 grid gap-4 md:grid-cols-2">
              <div>
                <Label>{t("ui.calculator.labels.automationCoverage")}</Label>
                <input
                  type="range"
                  min={0}
                  max={100}
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-3" aria-live="polite">
        <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-4">
          <div className="text-sm text-slate-400 break-words hyphens-auto">{t("ui.calculator.kpi.savings")}</div>
          <div className="mt-1 text-2xl font-bold text-emerald-400">{fmtC(out.Savings)}</div>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-4">
          <div className="text-sm text-slate-400 break-words hyphens-auto">{t("ui.calculator.kpi.roi")}</div>
          <div className="mt-1 text-2xl font-bold text-violet-400">{pctFormatter(out.ROI)}</div>
        </div>
        <div className="rounded-xl border border-slate-800 bg-slate-900/70 p-4">
          <div className="text-sm text-slate-400 break-words hyphens-auto">{t("ui.calculator.kpi.payback")}</div>
          <div className="mt-1 text-2xl font-bold text-sky-400">
            {!Number.isFinite(out.Payback) ? "—" : `${out.Payback.toFixed(1)} ${t("ui.calculator.kpi.months")}`}
          </div>
        </div>
      </div>
...
`}
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-3 text-sm">
          <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
            <div className="text-slate-400">{t("ui.calculator.breakdown.labor")}</div>
            <div className="mt-1 font-semibold">{fmtC(out.C_labor)}</div>
          </div>
          <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
            <div className="text-slate-400">{t("ui.calculator.breakdown.aiCost")}</div>
            <div className="mt-1 font-semibold">{fmtC(out.C_AI)}</div>
          </div>
          <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
            <div className="text-slate-400">{t("ui.calculator.breakdown.proposedTotal")}</div>
