  ...
  const handleNewsletterSubmit = async (e) => {
    ...
    try {
      ...
      if (!MAILCHIMP_API_KEY) {
        // No API key configured — show informational message instead of fake success
        setMessage({ type: "error", text: "Newsletter coming soon. Please check back later." });
        return;
      }

      // TODO: Implement Mailchimp API integration
      // Until implemented, inform the user rather than silently dropping their submission
      setMessage({ type: "error", text: "Newsletter subscription is not yet available." });
    } catch (error) {
      console.error(error);
