...
export default function MlopsCicdPage() {
  ...
  // SEO helpers
  const canonicalUrl = getCanonicalUrl(region, "/services/mlops-cicd");
  const canonicalBase = getCanonicalBase(region);
  const ogLocale = getOgLocale(region);
  const hreflangLinks = generateHreflangLinks("/services/mlops-cicd");
      ...
      <Helmet>
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "@type": "Service",
            "name": t("serviceMlops.meta.title"),
            "provider": {
              "@type": "Organization",
              "name": "Algorythmos",
              "url": canonicalBase
            },
            "description": t("serviceMlops.meta.description"),
            "url": canonicalUrl,
