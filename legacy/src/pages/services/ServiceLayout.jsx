/**
 * @deprecated This component is not currently imported anywhere.
 * Dedicated service pages (AgenticAutomationPage, DocumentIntelligencePage, etc.)
 * have their own layouts. ServiceDetailPage.jsx is used for dynamic fallback routes.
 * TODO: Evaluate for removal or consolidation with ServiceDetailPage.jsx
 */
...
import { getCanonicalUrl, getCanonicalBase, getOgLocale, generateHreflangLinks } from "../../app/utils/seoHelpers.js";
  ...
  return (
    <nav aria-label={t("ui.aria.breadcrumb")} className="text-sm mb-6">
      <ol className="flex flex-wrap gap-2 text-gray-400">
        <li><Link to={getRegionPath("/")} className="hover:text-white">{t("ui.breadcrumb.home")}</Link></li>
        <li>›</li>
        <li><Link to={getRegionPath("/services")} className="hover:text-white">{t("ui.breadcrumb.services")}</Link></li>
        <li>›</li>
...
export default function ServiceLayout({
  ...
  const serviceLd = useMemo(() => ({
    ...
    areaServed: region === "AU" ? "Australia" : region === "FR" ? "France" : "Worldwide",
    url: canonicalUrl,
    category: "Artificial Intelligence"
  }), [slug, title, meta, region, canonicalUrl]);

  const breadcrumbLd = useMemo(() => ({
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: t("ui.breadcrumb.home"), item: `${canonicalBase}/` },
      { "@type": "ListItem", position: 2, name: t("ui.breadcrumb.services"), item: `${canonicalBase}/services` },
      { "@type": "ListItem", position: 3, name: title, item: canonicalUrl }
    ]
  }), [slug, title, t, canonicalBase, canonicalUrl]);
    ...
    <div className="min-h-screen bg-black text-white overflow-hidden relative">
      ...
      <main className="pt-40 pb-24 px-6">
        <article className="max-w-5xl mx-auto grid lg:grid-cols-[1fr,280px] gap-10">
          <div>
            <Breadcrumbs title={title} t={t} getRegionPath={getRegionPath} />
            ...
            {!!faqs.length && (
              <section className="mt-12">
                <h2 className="text-2xl font-bold mb-4">{t("ui.serviceLayout.faqs")}</h2>
            ...
            <div className="mt-12 flex flex-wrap items-center gap-4">
              <Link
                to={getRegionPath(`/contact?service=${slug}`)}
              ...
              <Link to={getRegionPath("/services")} className="text-gray-400 hover:text-white underline">{t("ui.breadcrumb.backToServices")}</Link>
            </div>
          </div>
            ...
            <div className="sticky top-32 space-y-6">
              {!!sections.length && (
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <h3 className="font-semibold mb-3">{t("ui.serviceLayout.contents")}</h3>
              ...
              {!!related.length && (
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <h3 className="font-semibold mb-3">{t("ui.serviceLayout.relatedServices")}</h3>
                  <ul className="space-y-2 text-sm">
                    {related.map(r => (
                      <li key={r.path}>
                        <Link to={getRegionPath(r.path)} className="text-blue-300 hover:text-white">{r.title}</Link>
                      </li>
                    ))}
                  </ul>
                </div>
